...
{
    /*Аннуитетный метод кредитования.*/
    class Annuitentnya
    {
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("Loan options: Annuity method of lending".ToUpper());
        ...
        AgainThree:
            ...
            Annuitentnya creditpayment = new Annuitentnya(interestRate, creditAmount, creditTerm);      // Экземпляр класса Annuitentnya, который позволяет сделать расчеты кредитования по аннуитетному методу
            creditpayment.OutstandingBalance();                                                         // Вызов метода расчета кредитования по аннуитетному методу
            Console.WriteLine();
            Console.WriteLine(new string('-', 100));
